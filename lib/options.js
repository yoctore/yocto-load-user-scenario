...
var DEFAULT_JWT_HEADER      = 'x-jwt-token';
var DEFAULT_JWT_TOKEN_URL   = '/token/refresh';
/**
...
var options = [
  ...
  },
  {
    ...
    type          : String,
    group         : 'request',
    description   : [ 'If is defined enable jwt header support and use given header on request',
                      byDefault(DEFAULT_JWT_HEADER), optional() ].join(' ')
  },
  {
    name          : 'jwtUrl',
    alias         : 'x',
    type          : String,
    group         : 'request',
    description   : [ 'Url to use on scenario to get jwt token',
                      byDefault(DEFAULT_JWT_TOKEN_URL), optional() ].join(' ')
  },
  // Output property
  {
    name          : 'output',
...
var schema = joi.object().required().keys({
  ...
  request   : joi.object().optional().keys({
    ...
    jwt         : joi.string().optional().empty(null).default(DEFAULT_JWT_HEADER),
    jwtUrl      : joi.string().optional().empty(null).default(DEFAULT_JWT_TOKEN_URL)
  }).default({
    user        : DEFAULT_USER,
    time        : DEFAULT_TIME,
    limit       : DEFAULT_TIMEOUT,
    request     : REQUEST_PER_SECOND,
    jwt         : DEFAULT_JWT_HEADER,
    jwtUrl      : DEFAULT_JWT_TOKEN_URL
  }),
