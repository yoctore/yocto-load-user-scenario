...
var exportReport = function (report, toFile, filePath, last) {
  ...
  if (!last) {
    ...
    // parse all items push into storage
    async.eachSeries(_.flatten(report), function (report, next) {
      // maybe here print a legende
      content += '\n' + header(report.title, { 'padding-left' : 3 });

      // build
      _.every(report.result, function (citem) {
        // cloen item
        var item  = _.clone(citem);
        // build default table report
        var table = new Table({
          head  : [ 'Total Req.', 'Total err.', 'Total time (Ms)', 'RPS',
        ...
        // maybe here print a legende
        content += [ '\n', header(item.url, { 'padding-left' : 6 }, 157), '\n' ].join('');

        // push items
        table.push([
          item.totalRequests,
          item.totalErrors,
          (item.totalTimeSeconds * 1000).toFixed(2),
          item.rps,
          item.meanLatencyMs,
          item.maxLatencyMs,
          item.minLatencyMs,
