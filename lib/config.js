...
var isValid = function (command) {
  ...
  var config = [
    ...
    { 'Jwt header to use : ' : command.request.jwt || 'Disabled' },
    { 'Jwt token url to use : ' : command.request.jwt ? command.request.jwtUrl : 'Disabled' },
              ...
              inquirer.prompt({
                ...
                if (response.ready) {
                  ...
                  deferred.resolve({
                    ...
                    jwt               : command.request.jwt,
                    jwtUrl            : command.request.jwt ? command.request.jwtUrl : false
                  });
                } else {
  ...
  var entries = _.flatten(_.map(json.log.entries, function (entry) {
    ...
    return {
      ...
      entry : _.map(_.pick(entry, [ 'request' ]), function (request) {
        ...
        // default statement
        return _.pick(request, [ 'method', 'url', 'headers', 'queryString', 'postData' ]);
      })
    };
  }));
